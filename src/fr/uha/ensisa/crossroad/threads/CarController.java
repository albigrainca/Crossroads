...
public class CarController extends Thread {
    ...
    public CarController(List<Car> cars, Semaphore semaphore, TilePanel[][] grid, TrafficLightController trafficLightController) {
        this.cars = cars;
        ...
        try {
            while (true) {
                ...
                for (Car car : cars) {
                    ...
                    if (car.hasCrossedLight() || (car.getDirection() == 0 && trafficLightController.getTrafficLight1().isGreen()) || (car.getDirection() == 1 && trafficLightController.getTrafficLight2().isGreen())) {
                        car.move();
                        int newX = car.getX();
                        int newY = car.getY();
